...
def test_init_db(db_handler):
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='scraping_stats'")
...
def test_set_stat_exception(mocker, db_handler):
    ...
    mock_conn = MagicMock()
    mock_conn.__enter__.return_value = mock_conn  # Ensure context manager protocol is supported
    
    # Create a Mock for cursor
    mock_cursor = Mock()
    mock_cursor.execute.side_effect = sqlite3.IntegrityError  # Set side effect to raise IntegrityError
    
    # Set cursor() to return mock_cursor
    mock_conn.cursor.return_value = mock_cursor
    
    # Patch 'sqlite3.connect' in 'sqlite_handler.py' to return the mock connection
    mocker.patch('product_school_scraper.database.sqlite_handler.sqlite3.connect', return_value=mock_conn)
