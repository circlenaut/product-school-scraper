...
def test_verify_pages_with_files(mocker, caplog):
    ...
    mocker.patch(
        "product_school_scraper.services.verification_service.Path.exists",
        return_value=True,
    )
    mocker.patch(
        "product_school_scraper.services.verification_service.Path.is_dir",
        return_value=True,
    )
    ...
    mocker.patch(
        "product_school_scraper.services.verification_service.Path.iterdir",
        return_value=[mock_subdir],
    )

    # Mock PdfReader to return a reader with one page
    mock_pdf_reader = mocker.patch(
        "product_school_scraper.services.verification_service.pypdf.PdfReader"
    )
    ...
    def open_mock(path, mode="r", *args, **kwargs):
        ...
        raise ValueError(f"Unexpected mode for open: {mode}")

    mocker.patch(
        "product_school_scraper.services.verification_service.open",
        side_effect=open_mock,
    )
    ...
    assert (
        "Valid PDF: " in caplog.text
    )  # The actual path will be a Mock object, so not checking exact path
...
def test_verify_pages_invalid_directory(mocker, caplog):
    ...
    mock_path = mocker.patch(
        "product_school_scraper.services.verification_service.Path"
    )
    ...
    mock_logger_error = mocker.patch(
        "product_school_scraper.services.verification_service.logger.error"
    )
    ...
    mock_logger_error.assert_called_once_with(
        "Directory 'invalid_pages_dir' does not exist or is not a directory."
    )
...
def test_verify_pages_with_invalid_pdf(mocker, caplog):
    ...
    mock_logger_debug = mocker.patch(
        "product_school_scraper.services.verification_service.logger.debug"
    )
    ...
    mock_logger_debug.assert_any_call(
        f"PDF read error on '{mock_pdf_file_path}': Invalid PDF content"
    )
...
def test_is_valid_pdf_exception(mocker):
    ...
    mock_logger_debug.assert_called_once_with(
        f"PDF read error on '{mock_pdf_path}': Read error"
    )
